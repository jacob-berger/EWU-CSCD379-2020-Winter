<template>
    <div>
        <button class="button is-primary" @click="createGroup()">Create New</button>
        <table class="table">
            <thead>
                <tr>
                    <th>Id</th>
            ...
            <tbody>
                <tr v-for="group in groups" :id="group.id">
                    ...
                    <td>{{group.title}}</td>
                    <td>
                        <button class="button is-primary" @click="setGroup(group)">Edit</button>
...
<script lang="ts">
    ...
    export default class GroupsComponent extends Vue{
        ...
        async deleteGroup(group: Group) {
            let groupClient = new GroupClient();
            if(confirm(`Confirm deletion of ${group.title}?`)){
