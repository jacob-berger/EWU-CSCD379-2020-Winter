<template>
    <div>
        <button class="button is-primary" @click="createGift()">Create New</button>
        <table class="table">
            <thead>
                <tr>
                    <th>Id</th>
            ...
            <tbody>
                <tr v-for="gift in gifts" :id="gift.id">
                    ...
                    <td>{{gift.userId}}</td>
                    <td>
                        <button class="button is-primary" @click="setGift(gift)">Edit</button>
...
<script lang="ts">
    ...
    export default class GiftsComponent extends Vue{
        ...
        async deleteGift(gift: Gift){
            let giftClient = new GiftClient();
            if(confirm(`Confirm deletion of ${gift.title}?`)){
