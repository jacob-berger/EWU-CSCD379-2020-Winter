<template>
    <div>
        <button class="button is-primary" @click="createUser()">Create New</button>
        <table class="table">
            <thead>
                <tr>
                    <th>Id</th>
            ...
            <tbody>
                <tr v-for="user in users" :id="user.id">
                    ...
                    <td>{{user.lastName}}</td>
                    <td>
                        <button class="button is-primary" @click="setUser(user)">Edit</button>
...
<script lang="ts">
    ...
    export default class UsersComponent extends Vue{
        ...
        async deleteUser(user: User){
            let userClient = new UserClient();
            if(confirm(`Confirm deletion of ${user.firstName} ${user.lastName}?`)){
